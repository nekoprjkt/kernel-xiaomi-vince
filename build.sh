...
ANYKERNEL_REPO="https://github.com/nekoprjkt/Anykernel3"
...
# Check If compilation is success
    if ! [ -f "${KERN_IMG}" ]; then
        ...
        tg_cast "Build for ${DEVICE} <b>failed</b> in $((DIFF / 60)) minute(s) and $((DIFF % 60)) second(s)! Check Instance for errors @OnTheWay69"
        exit 1
    fi
}
